...
* 16/10/13 Alex.  If variable plUnattendedExecution = .T. use DebugOut instead of MessageBox
		IF TYPE('plUnattendedExecution') = 'L' AND plUnattendedExecution
			TRY
				DebugOut(m.cTitle + ' ---> ' + m.cMessage)
			CATCH
			ENDTRY
			RETURN "IGNORE"
		ELSE
